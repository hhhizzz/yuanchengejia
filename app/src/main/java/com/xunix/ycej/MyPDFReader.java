...
public class MyPDFReader extends Activity implements OnItemClickListener, OnClickListener, PDFReaderListener, PDFThumbListener
{
    ...
    public void onCreate(Bundle savedInstanceState)
    {
        ...
        m_layout = (RelativeLayout) LayoutInflater.from(this).inflate(R.layout.my_reader, null);
        m_reader = (PDFReader)m_layout.findViewById(R.id.view);
        m_thumb = (PDFThumbView)m_layout.findViewById(R.id.thumbs);

        m_doc = new Document();
        m_doc.Open(filepath, null );
        Log.i("filePathReader",filepath);
        m_reader.PDFOpen(m_doc, false, this);
        m_thumb.thumbOpen(m_doc, this);

//        m_vFiles = new PDFGridView(this, null);
//        m_vFiles.PDFSetRootPath(Environment.getExternalStorageDirectory().getPath());
...
//        setContentView(m_vFiles);
    ...
    private void onEdit()
    {
        RelativeLayout layout = (RelativeLayout) LayoutInflater.from(this).inflate(R.layout.dlg_note, null);
        ...
        builder.setPositiveButton("OK", new DialogInterface.OnClickListener()
        {
            ...
            public void onClick(DialogInterface dialog, int which)
            {
                if( rad_group.getCheckedRadioButtonId() == R.id.rad_copy )
                    Toast.makeText(MyPDFReader.this, "todo copy text:" + sel_text, Toast.LENGTH_SHORT).show();
                else if( m_reader.PDFCanSave() )
                {
                    ...
                    if( !ret )
                        Toast.makeText(MyPDFReader.this, "add annotation failed!", Toast.LENGTH_SHORT).show();
                }
                else
                    Toast.makeText(MyPDFReader.this, "can't write or encrypted!", Toast.LENGTH_SHORT).show();
                onSelect();
                dialog.dismiss();
            }});
        builder.setNegativeButton("Cancel", new DialogInterface.OnClickListener()
